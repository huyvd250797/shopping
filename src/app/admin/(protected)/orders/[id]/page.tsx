...
import { ORDER_STATUS_TRANSITIONS, orderStatusClass, orderStatusLabel } from "@/features/orders/status";
  ...
  return (
    <>
      <div className="admin-page-head order-detail-head">
        ...
        <span className={`order-status-badge order-status-large ${orderStatusClass(order.status)}`}>{orderStatusLabel(order.status)}</span>
      </div>
      ...
      <div className="order-management-grid">
        <section className="panel order-workflow-panel">
          ...
          {nextStatuses.length === 0 ? (
            <div className="order-terminal-state">Đơn đang ở trạng thái kết thúc <strong>{orderStatusLabel(order.status)}</strong>, không còn bước chuyển tiếp.</div>
          ) : (
            <form action={transitionOrderAction} className="order-admin-form">
              ...
              <div className="order-transition-hint">
                Từ <strong>{orderStatusLabel(order.status)}</strong> có thể chuyển tới: {nextStatuses.map(orderStatusLabel).join(" • ")}.
              </div>
